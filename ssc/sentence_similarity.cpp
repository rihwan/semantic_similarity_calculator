...
float SentenceSimilarityLi2006::compute_similarity(const std::string & s1, const std::string & s2, WordnetExtended::UndirectedGraph & graph)
{
...
void SentenceSimilarityLi2006::build_sim_vector(
    WordnetExtended & wne,
    ...
    for (std::vector<std::string>::const_iterator it_corpus = corpus.begin(); it_corpus != corpus.end(); ++it_corpus) {
        ...
        if (std::find(s.begin(), s.end(), *it_corpus) == s.end()) {
            ...
            for (std::vector<std::string>::const_iterator it_s = s.begin(); it_s != s.end(); ++it_s) {
                WordnetExtended::vertex_t v_w_s, v_w_corpus;
...
float SentenceSimilarityLi2006::calc_cos_similarity(
    const std::vector<float> & sim1,
    const std::vector<float> & sim2)
{
